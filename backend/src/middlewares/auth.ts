...
export const auth = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    return res.status(statusCode.UNAUTHORIZED).send({ success: false, message: 'Неавторизованный доступ' });
  }
  ...
  verifyJwt(token)
    ...
    .catch((err) => res.status(statusCode.NOT_FOUND).send({ success: false, message: err.message })
    );
};
